...
export default function ContactForm() {
  ...
  return (
    // <div className="c-form"> の外側をフラグメントで囲います
    <>
            ...
            <div className="form-step" data-step="9" style={{ display: currentStep === 9 ? 'block' : 'none' }}>
              ...
              <div className="hs-privacy_policy">
                <label className="hs-form-booleancheckbox">
                  ...
                  <span>
                    {/* <a> タグを <span> に変更し、
                      href/data-lity の代わりに onClick を使用します。
                      カーソルと下線でクリック可能であることを示します。
                    */}
                    <span 
      ...
      {/* isPpModalOpen が true の場合にモーダルコンポーネントを描画します。
        onClose イベントで状態を false に戻します。
        (div.c-form の兄弟要素として配置)
      */}
      {isPpModalOpen && (
        <PrivacyPolicyModal onClose={() => setIsPpModalOpen(false)} />
      )}
    </> // ここがフラグメントの閉じタグです
  );
}